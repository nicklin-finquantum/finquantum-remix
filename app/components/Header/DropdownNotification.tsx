...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    ...
    if (user?.notify !== undefined) setNotifying(user?.notify);

    websocketService.connectNotificationSocket(user?.id, (data) => {
      if (data.new) {
        setNotifying(true);
        const updatedUser = { ...user, notify: true }; // Create a new user object with updated notify
      ...
      const newNotification = {
        ...
        applicationLink: data.reportName
          ? `/admin/service#reports?s=${encodeURIComponent(data.reportName)}`
    ...
    return () => {
      if (websocketService) {
        websocketService.closeNotificationSocket();
      }
    };
  }, [user, setUser]);
          ...
          {notifications.map((notification, index) => (
            <li key={index}>
              <Link
                ...
                to={notification?.applicationLink}
                onClick={handleNotificationClick} // Close dropdown on click
              >
